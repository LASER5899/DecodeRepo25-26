...
public class VerticalSlide {
    ...
    private VerticalSlideState state = VerticalSlideState.HOME;
    ...
    public void update(boolean upPressed, boolean downPressed, boolean resetPressed) {
        switch(state) {
            ...
            case MID:
                ...
                else if (downPressed) state = VerticalSlideState.HOME;
                else if (resetPressed) state = VerticalSlideState.RESETTING;
                break;
            case HIGH:
                slide.setTargetPosition(5024);
                slide.setPower(0.7);
                if (downPressed) state = VerticalSlideState.HOME;
                else if (resetPressed) state = VerticalSlideState.RESETTING;
                break;
            case RESETTING:
                slide.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER); // if encoder skips or drifts or smth
                slide.setPower(-0.5);
                if (!resetPressed) state = VerticalSlideState.HOME; // assumes a hold to reset
                break;
