...
//@Disabled
public class OLD_LASER_TELEOP_CLASSTEST extends LinearOpMode {
            ...
            */
            ...
            switch (vSlideMotorState) {
                case 0:
                    ...
                    robot.slideVertical.setPower(1.0);
                    robot.slideVertical.setTargetPosition(78);
                    break;
                case 1:
                    ...
                    robot.slideVertical.setPower(1.0);
                    robot.slideVertical.setTargetPosition(2669);
                    break;
                case 2:
                    if (C_VERT_SLIDE_DWN && !PREV_C_VERT_SLIDE_DWN) {vSlideMotorState = 1;}
                    robot.slideVertical.setPower(0.7);
                    robot.slideVertical.setTargetPosition(5024);
                    break;
            }
            ...
            if (robot.slideHorizontal.getCurrentPosition() > -200) {
                robot.lights.setColor(LEDlights.yellowColor);
            } else if (robot.slideHorizontal.getCurrentPosition() < -1500) {
                robot.lights.setColor(LEDlights.redFireColor);
            } else {
                robot.lights.setColor(LEDlights.blueRayColor);
            }
            ...
            if (C_IN_SERVO_TRANSF && !C_INTAKE) {
                robot.intakeServo.setPosition(1.0); //pick up or transfer
            } else if (C_SPIT) {
                robot.intakeServo.setPosition(0.0); //spit out
            } else {
                robot.intakeServo.setPosition(0.5); //not moving
            }

            // BLOCK PUSH SERVO CONTROLS
            if (C_BPUSHSERVO) {
                robot.blockPushServo.setPosition(0.8);
            } else if (C_BPUSHSERVO_LOAD) {
                robot.blockPushServo.setPosition(1.0);
            } else {
                robot.blockPushServo.setPosition(0.0);
            }
            ...
            if (C_INTAKE || C_SPIT) {
                ...
                robot.wristMotor.setPower(0.2);
                robot.wristMotor.setTargetPosition(130); //avoid
            } else if (robot.slideHorizontal.getCurrentPosition() < -300) {
                robot.wristMotor.setPower(0.5);
                robot.wristMotor.setTargetPosition(400); //idle
            } else {
                robot.wristMotor.setPower(0.7);
                robot.wristMotor.setTargetPosition(15); //in
            ...
            telemetry.addData("Servo Position", "%4.2f", robot.outtakeServo.getPosition());
            telemetry.addData("Wrist Position", "%4.2f", (double)robot.wristMotor.getCurrentPosition()); //??
            telemetry.addData("Wrist Power", "%4.2f", robot.wristMotor.getPower()); //??
